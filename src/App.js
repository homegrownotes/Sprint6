...
function App() {
  ...
  function goNext() {
    // Declare the function "goNext".
    if (position < data.length) {
      // If the position number is less than the total number of contents...
      setPosition(position + 1); // Add "position" to the following content.
    }
    if (position >= data.length) {
      // If the position number is > or = the total number of contents, "it means that it is the last content".
  ...
  function goBack() {
    ...
    if (position === 1) {
      // If the position number is = to 1 "it is in the first content"...
      setPosition(4); // So we send it to the last content.
    }
  }

  function handleStartButtonClick() {
    // Add event handler to hide Welcome screen.
    ...
    <>
      {showWelcomeScreen === true ? (
        ...
        <div>
          ...
          <WelcomeP>
            Recorre la història del nostre protagonista a través dels dos botons
            interactius “Anterior” i “Següent”. ¿Preparat? Donçs comença...😀
          </WelcomeP>
          <ButtonStart onClick={handleStartButtonClick}>Començar</ButtonStart>
        </div>
      ) : (
        <>
          <Container background={data[position - 1].img}>
            <div>
